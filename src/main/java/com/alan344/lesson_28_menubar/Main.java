...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        customMenuItem.setContent(button1);

        CustomMenuItem customMenuItem2 = new CustomMenuItem();
        ProgressBar progressBar = new ProgressBar(0.5);
        customMenuItem.setContent(progressBar);

        CustomMenuItem customMenuItem3 = new CustomMenuItem();
        customMenuItem3.setStyle("-fx-background-color: blue");
        HBox hBox = new HBox();
        hBox.getChildren().addAll(new Button("b1"), new Button("b2"));
        customMenuItem.setContent(hBox);

        menu1.getItems().addAll(menuItem1, menuItem2, menuItem3, customMenuItem, customMenuItem2, customMenuItem3);
